...
<?php
    ...
    // $sql = "SELECT b.mypostID, b.postTitle, b.postContents, b.regTime, b.postView FROM myBoard b JOIN myMember m ON(b.mypostID = m.mypostID) ";
    $sql = "SELECT b.mypostID, b.postTitle, b.postContents, m.youName, b.regTime, b.postView FROM myBoard b JOIN myMember m ON(b.myMemberID = m.myMemberID) ";
    
    switch($searchOption){
        case "title" : 
            $sql .= "WHERE b.postTitle LIKE '%{$searchKeyword}%' ORDER BY mypostID DESC ";
            break;
        case "content" : 
            $sql .= "WHERE b.postContents LIKE '%{$searchKeyword}%' ORDER BY mypostID DESC ";
            break;
        case "name" : 
            $sql .= "WHERE m.youName LIKE '%{$searchKeyword}%' ORDER BY mypostID DESC ";
            break;
    };
...
<?php
    ...
    if($totalCount > 0){
        ...
        for($i=1; $i <= $count; $i++){
            ...
            echo "<tr>";
            echo "<td>".$info['mypostID']."</td>";
            echo "<td><a href='postView.php?mypostID={$info['mypostID']}'>".$info['postTitle']."</a></td>";
